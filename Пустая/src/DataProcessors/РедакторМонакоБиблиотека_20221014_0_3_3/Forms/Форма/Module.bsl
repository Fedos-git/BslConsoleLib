...
Процедура ПриОткрытии(Отказ)   
	...
	укМонако = ПолучитьФорму(ПолучитьПолноеИмяФормы("bsl_console"), ПараметрыРедактора, ЭтотОбъект);
	укМонако.ПриОткрытии(Ложь);
...
Функция View()
	...
	Если Элементы.ХТМЛРедактор.Документ <> Неопределено Тогда
		Возврат Элементы.ХТМЛРедактор.Документ.defaultView;
	Иначе
...
Процедура ПрименитьНастройкиРедактора(ВыбранныеНастройки = Неопределено)
	...
	Если ПараметрыФормы.ДоступенWebKit Тогда
		View().minimap(НастройкиРедактора.КартаКода);
		Если НастройкиРедактора.СтрокаСостояния Тогда
			View().showStatusBar();
		Иначе	
			View().hideStatusBar();
		КонецЕсли;
		Тема = НастройкиРедактора.Тема;
		Если укМонако.РедакторВРежимеЗапроса() Тогда
			Тема = Тема + "-query"	
		КонецЕсли;
		View().setTheme(Тема);
		View().switchLang(НастройкиРедактора.Язык);   
		Если НастройкиРедактора.НомераСтрок Тогда
			View().showLineNumbers();
		Иначе	
			View().hideLineNumbers();
		КонецЕсли;   
		View().renderWhitespace(НастройкиРедактора.ОтображатьПробелы);
...
Функция ПолучитьПолноеИмяФормы(ИмяФормы)
	...
	ПозицияТочки = СтрНайти(ЭтотОбъект.ИмяФормы, ".", НаправлениеПоиска.СКонца);
	Результат = Лев(ЭтотОбъект.ИмяФормы, ПозицияТочки) + ИмяФормы;
	Возврат Результат;
